...
class RealtimeService:
    ...
    def wire(self) -> None:
        ...
        self._bus.subscribe(
            UserProfileUpdated, self._on_user_profile_updated,
        )
        self._bus.subscribe(
            SpaceMemberProfileUpdated, self._on_space_member_profile_updated,
        )
        self._bus.subscribe(
            PairingAcceptReceived, self._on_pairing_accept_received,
        )
        self._bus.subscribe(
            PairingConfirmed, self._on_pairing_confirmed,
        )
        self._bus.subscribe(
            PairingAborted, self._on_pairing_aborted,
        )
        self._bus.subscribe(
            PairingIntroReceived, self._on_pairing_intro_received,
        )
        self._bus.subscribe(
            AutoPairRequestIncoming, self._on_auto_pair_requested,
        )
        ...
        self._bus.subscribe(
            SchedulePollResponded, self._on_schedule_responded,
        )
        self._bus.subscribe(
            SchedulePollFinalized, self._on_schedule_finalized,
        )
        ...
        self._bus.subscribe(
            NotificationReadChanged, self._on_notification_read_changed,
        )
        self._bus.subscribe(BazaarBidPlaced, self._on_bazaar_bid_placed)
        self._bus.subscribe(
            BazaarListingExpired, self._on_bazaar_listing_closed,
        )
        self._bus.subscribe(
            BazaarListingCreated, self._on_bazaar_listing_created,
        )
        self._bus.subscribe(
            BazaarListingUpdated, self._on_bazaar_listing_updated,
        )
        self._bus.subscribe(
            BazaarListingCancelled, self._on_bazaar_listing_cancelled,
        )
        self._bus.subscribe(
            BazaarOfferAccepted, self._on_bazaar_offer_accepted,
        )
        self._bus.subscribe(
            BazaarOfferRejected, self._on_bazaar_offer_rejected,
        )
        self._bus.subscribe(
            BazaarBidWithdrawn, self._on_bazaar_bid_withdrawn,
        )
        self._bus.subscribe(DmMessageCreated, self._on_dm_message_created)
        self._bus.subscribe(
            HouseholdConfigChanged, self._on_household_config_changed,
        )
    ...
    async def _on_space_member_profile_updated(
        ...
        await self._broadcast_space(event.space_id, {
    ...
    async def _on_pairing_accept_received(
        self, event: PairingAcceptReceived,
    ) -> None:
    ...
    async def _on_pairing_intro_received(
        self, event: PairingIntroReceived,
    ) -> None:
        await self._broadcast_household({
    ...
    async def _on_auto_pair_requested(
        self, event: AutoPairRequestIncoming,
    ) -> None:
        await self._broadcast_household({
    ...
    async def _on_space_mod_queued(self, event: SpaceModerationQueued) -> None:
        item = event.item
        await self._broadcast_space(item.space_id, {
    ...
    async def _on_space_join_requested(
        ...
        await self._ws.broadcast_to_users(admin_ids, {
    ...
    async def _on_task_assigned(self, event: TaskAssigned) -> None:
        await self._ws.broadcast_to_user(event.assigned_to, {
            "type": "task.assigned",
    ...
    async def _on_task_deadline(self, event: TaskDeadlineDue) -> None:
        for assignee in (event.task.assignees or ()):
            await self._ws.broadcast_to_user(assignee, {
    ...
    async def _on_schedule_responded(
        self, event: SchedulePollResponded,
    ) -> None:
        payload = {
    ...
    async def _on_schedule_finalized(
        self, event: SchedulePollFinalized,
    ) -> None:
        payload = {
    ...
    async def _on_household_config_changed(
        self, event: HouseholdConfigChanged,
    ) -> None:
    ...
    async def _on_notification_new(self, event: NotificationCreated) -> None:
        await self._ws.broadcast_to_user(event.user_id, {
    ...
    async def _on_notification_read_changed(
        self, event: NotificationReadChanged,
    ) -> None:
        await self._ws.broadcast_to_user(event.user_id, {
    ...
    async def _on_bazaar_listing_created(
        self, event: BazaarListingCreated,
    ) -> None:
        await self._broadcast_household({
    ...
    async def _on_bazaar_listing_updated(
        self, event: BazaarListingUpdated,
    ) -> None:
        await self._broadcast_household({
    ...
    async def _on_bazaar_listing_cancelled(
        self, event: BazaarListingCancelled,
    ) -> None:
        await self._broadcast_household({

...
def register_routes(app: web.Application, *, admin_ui_dir: Path, media_dir: str) -> None:
    ...
    app.router.add_view(
        "/admin/api/clients/{instance_id}/{action}", AdminClientActionView,
    )
    app.router.add_view("/admin/api/spaces",        AdminSpaceCollectionView)
    app.router.add_view(
        "/admin/api/spaces/{space_id}/{action}", AdminSpaceActionView,
    )
    app.router.add_view("/admin/api/policy",   AdminPolicyView)
    app.router.add_view("/admin/api/branding", AdminBrandingView)
    app.router.add_view(
        "/admin/api/branding/header-image", AdminBrandingHeaderImageView,
    )
    app.router.add_view("/admin/api/reports", AdminReportCollectionView)
    app.router.add_view(
        "/admin/api/reports/{report_id}/review", AdminReportReviewView,
    )
    app.router.add_view("/admin/api/appeals", AdminAppealCollectionView)
    app.router.add_view(
        "/admin/api/appeals/{appeal_id}/decide", AdminAppealDecideView,
    )
    app.router.add_view("/admin/api/audit",   AdminAuditView)

    app.router.add_view("/admin/api/cluster",       AdminClusterCollectionView)
    app.router.add_view(
        "/admin/api/cluster/peers", AdminClusterPeerCollectionView,
    )
    app.router.add_view(
        "/admin/api/cluster/peers/{node_id}", AdminClusterPeerDetailView,
    )
    app.router.add_view(
        "/admin/api/cluster/peers/{node_id}/ping", AdminClusterPeerPingView,
    )

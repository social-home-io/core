...
async def test_feed_list_with_before(client):
    ...
    h = _auth(client._tok)
    await client.post("/api/feed/posts", json={"type": "text", "content": "A"}, headers=h)
    await client.post("/api/feed/posts", json={"type": "text", "content": "B"}, headers=h)
...
async def test_feed_comment_list_empty(client):
    ...
    h = _auth(client._tok)
    r = await client.post("/api/feed/posts", json={"type": "text", "content": "X"}, headers=h)
    pid = (await r.json())["id"]

...
async def test_create_sticky(client):
    ...
    r = await client.post("/api/stickies", json={"content": "Remember this"},
...
async def test_list_stickies(client):
    ...
    await client.post("/api/stickies", json={"content": "Note"},
                       headers=_auth(client._tok))
...
async def test_update_sticky(client):
    ...
    r = await client.post("/api/stickies", json={"content": "v1"},
                           headers=_auth(client._tok))
    sid = (await r.json())["id"]
    r2 = await client.patch(f"/api/stickies/{sid}",
                              json={"content": "v2"}, headers=_auth(client._tok))
    assert r2.status == 200


async def test_delete_sticky(client):
    """DELETE /api/stickies/{id} removes it."""
    r = await client.post("/api/stickies", json={"content": "tmp"},
                           headers=_auth(client._tok))
    sid = (await r.json())["id"]

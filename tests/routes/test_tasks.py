...
async def test_create_task_list(client):
    ...
    r = await client.post("/api/tasks/lists", json={"name": "Chores"}, headers=_auth(client._tok))
    assert r.status == 201
    body = await r.json()
...
async def test_list_task_lists(client):
    ...
    await client.post("/api/tasks/lists", json={"name": "Work"}, headers=_auth(client._tok))
...
async def test_create_task(client):
    ...
    r = await client.post("/api/tasks/lists", json={"name": "L"}, headers=_auth(client._tok))
    lid = (await r.json())["id"]
    r2 = await client.post(f"/api/tasks/lists/{lid}/tasks",
                            json={"title": "Buy milk"}, headers=_auth(client._tok))
...
async def test_list_tasks(client):
    ...
    r = await client.post("/api/tasks/lists", json={"name": "L"}, headers=_auth(client._tok))
    lid = (await r.json())["id"]
    await client.post(f"/api/tasks/lists/{lid}/tasks",
                       json={"title": "T"}, headers=_auth(client._tok))

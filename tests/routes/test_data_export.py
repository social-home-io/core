...
async def test_export_self_includes_authored_post(client):
    ...
    assert any(
        p.get("content") == "exported"
        for p in body["tables"]["feed_posts"]
    )
...
async def test_export_user_admin_succeeds(client):
    ...
    r = await client.get(
        "/api/users/bob-id/export", headers=_auth(client._tok),
    )
    assert r.status == 200
    body = json.loads(await r.read())
...
async def test_export_user_non_admin_403(client):
    ...
    r = await client.get(
        "/api/users/some-other-id/export", headers=_auth(raw),
    )
    assert r.status == 403

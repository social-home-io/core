...
async def test_feed_errors_create_post_empty_content(client):
    ...
    r = await client.post(
        "/api/feed/posts", json={"type": "text", "content": "   "},
...
async def test_feed_errors_create_post_bad_type(client):
    ...
    r = await client.post(
        "/api/feed/posts", json={"type": "bogus", "content": "x"},
        headers=_auth(client._admin_tok))
    assert r.status == 422
...
async def test_feed_errors_edit_nonexistent_post(client):
    ...
    r = await client.patch(
        "/api/feed/posts/nonexistent", json={"content": "x"},
        headers=_auth(client._admin_tok))
    assert r.status == 404
...
async def test_feed_errors_reaction_empty_emoji(client):
    ...
    r = await client.post(
        "/api/feed/posts", json={"type": "text", "content": "x"},
        headers=_auth(client._admin_tok))
    pid = (await r.json())["id"]
    r2 = await client.post(
        f"/api/feed/posts/{pid}/reactions", json={"emoji": ""},
        headers=_auth(client._admin_tok))
...
async def test_feed_errors_comment_on_deleted_post(client):
    ...
    r = await client.post(
        ...
        headers=_auth(client._admin_tok))
    pid = (await r.json())["id"]
    await client.delete(f"/api/feed/posts/{pid}",
                         headers=_auth(client._admin_tok))
    r2 = await client.post(
        f"/api/feed/posts/{pid}/comments", json={"content": "late"},
        headers=_auth(client._admin_tok))
...
async def test_feed_errors_edit_missing_content_field(client):
    ...
    r2 = await client.patch(
        f"/api/feed/posts/{pid}", json={"title": "no content key"},
        headers=_auth(client._admin_tok))
    assert r2.status == 422
...
async def test_feed_errors_feed_pagination_limit_clamp(client):
    ...
    r = await client.get(
        "/api/feed?limit=999", headers=_auth(client._admin_tok))
...
async def test_space_errors_update_config_non_admin(client):
    ...
    await client.post(
        ...
        headers=_auth(client._admin_tok))
    r2 = await client.patch(
        f"/api/spaces/{sid}", json={"name": "Hijacked"},
        headers=_auth(client._member_tok))
    assert r2.status == 403

...
def _make_remote(iid: str, status: PairingStatus = PairingStatus.CONFIRMED) -> RemoteInstance:
...
def test_ice_candidate_must_start_with_candidate_prefix_s7():
    assert SyncSessionManager.validate_ice_candidate("candidate:1 1 UDP 2 1.2.3.4 1234 typ host") is True
...
async def test_clamp_request_more_clamps_limit_to_200():
    mgr = SyncSessionManager(_FakeFedRepo())
    out = await mgr.clamp_request_more({
        "space_id": "sp-1", "resource": "posts", "limit": 9999,
    })
    assert out is not None
...
async def test_clamp_request_more_handles_bad_int():
    mgr = SyncSessionManager(_FakeFedRepo())
    assert await mgr.clamp_request_more({
        "space_id": "sp-1", "resource": "posts", "before_seq": "not-a-number",
    }) is None
...
async def test_begin_session_blocks_when_rate_limited_s6():
    ...
    decision = await mgr.begin_session(
        sync_id=new_sync_id(), space_id="sp-1",
        requester_instance_id="alice", provider_instance_id="me",
    )
...
async def test_begin_session_silently_drops_non_member_s1():
    ...
    d = await mgr.begin_session(
        sync_id="s1", space_id="sp-1",
        requester_instance_id="hostile", provider_instance_id="me",
    )
...
async def test_apply_answer_rejects_wrong_origin_s14():
    ...
    ok = await mgr.apply_answer(sync_id="s1", sdp_answer="v=0\r\n", from_instance="hostile")
    assert ok is False

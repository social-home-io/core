...
def _space(space_id: str) -> Space:
    return Space(
        id=space_id, name=space_id, owner_instance_id="self",
        owner_username="admin", identity_public_key="aa" * 32,
        config_sequence=0, features=SpaceFeatures(),
...
def _peer(instance_id: str) -> SimpleNamespace:
    return SimpleNamespace(
        id=instance_id, status=PairingStatus.CONFIRMED,
    )
...
async def test_on_pairing_confirmed_enqueues_for_shared_spaces(bus, queue):
    ...
    sched = SpaceSyncScheduler(
        bus=bus, federation=fed,
        federation_repo=_FakeFedRepo([_peer("peer-a")]),
        space_repo=_FakeSpaceRepo(
            spaces_by_type=spaces, members_by_space=members,
        ),
    ...
    beg = [s for s in fed.sent
           if s["type"] == FederationEventType.SPACE_SYNC_BEGIN]
    assert len(beg) == 1
...
async def test_enqueue_sync_for_space_sends_begin(bus, queue):
    ...
    sched = SpaceSyncScheduler(
        ...
        federation_repo=_FakeFedRepo([]),
        space_repo=_FakeSpaceRepo(
            spaces_by_type={}, members_by_space={},
        ),
        queue=queue, own_instance_id="self",
    )
    await queue.start()
    try:
        await sched.enqueue_sync_for_space(
            space_id="sp-1", peer_instance_id="peer-a",
        )
        await asyncio.sleep(0.05)
    finally:
        await queue.stop()
...
async def test_start_stop_idempotent(bus, queue):
    sched = SpaceSyncScheduler(
        bus=bus, federation=_FakeFederation(),
        federation_repo=_FakeFedRepo([]),

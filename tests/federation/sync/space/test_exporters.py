...
async def test_bans_exporter():
    ...
    ex = BansExporter(_FakeSpaceRepo(bans=[
...
async def test_posts_exporter_serialises_enums_and_datetimes():
    ...
    post = Post(
        id="p-1", author="u-1", type=PostType.TEXT,
...
async def test_tasks_archived_filters_to_done():
    ...
    now = datetime.now(timezone.utc)
    active = Task(
        id="t-1", list_id="l", title="X", status=TaskStatus.TODO,
        position=0, created_by="u-1", created_at=now, updated_at=now,
    )
    done = Task(
        id="t-2", list_id="l", title="Y", status=TaskStatus.DONE,
        position=1, created_by="u-1", created_at=now, updated_at=now,
    )
    class _Repo:
        async def list_by_space(self, space_id):
            return [active, done]
...
async def test_pages_exporter():
    ...
    page = Page(
        id="pg-1", title="Welcome", content="Hello",
        created_by="u-1",
...
async def test_calendar_exporter_serialises_datetimes():
    ...
    event = CalendarEvent(
        id="e-1", calendar_id="cal-1", summary="Sync meeting",
...
async def test_gallery_exporter_emits_albums_then_items():
    ...
    album = GalleryAlbum(
        id="a-1", space_id="sp-1", owner_user_id="u-1", name="Trip",
    )
    item = GalleryItem(
        id="i-1", album_id="a-1", uploaded_by="u-1",
        item_type="photo", url="/m/x.jpg",

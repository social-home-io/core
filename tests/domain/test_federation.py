...
def test_federation_event():
    ...
    e = FederationEvent(
        ...
        from_instance="a", to_instance="b", timestamp="now",
        payload={"key": "val"}, space_id="s1",
    )
...
def test_decrypted_payload():
    ...
    dp = DecryptedPayload(
        event_type=FederationEventType.DM_MESSAGE, payload={"x": 1},
    )
...
def test_federation_envelope():
    ...
    env = FederationEnvelope(
        msg_id="m1", event_type=FederationEventType.PAIRING_ACCEPT,
        from_instance="a", to_instance="b", timestamp="now",

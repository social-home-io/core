...
async def test_space_post_created_indexes_with_space_scope(env):
    svc, bus, _ = env
    await bus.publish(SpacePostCreated(post=_post("sp1", "secret recipe"), space_id="sp-A"))
...
async def test_search_filters_by_scope(env):
    ...
    await bus.publish(SpacePostCreated(post=_post("sp1", "needle in space"), space_id="sp-A"))
...
async def test_page_created_event_indexes_page(env):
    ...
    await bus.publish(PageCreated(
        page_id="page-1", space_id=None,
        title="Grocery notes", content="flour sugar butter",
    ))
...
async def test_page_updated_event_replaces_index_entry(env):
    ...
    svc, bus, _ = env
    await bus.publish(PageCreated(
        page_id="page-2", space_id=None,
        title="t", content="original text",
    ))
    await bus.publish(PageUpdated(
        page_id="page-2", space_id=None,
        title="t", content="refreshed body",
    ))
...
async def test_page_deleted_event_removes_index_entry(env):
    ...
    svc, bus, _ = env
    await bus.publish(PageCreated(
        page_id="page-3", space_id=None,
        title="t", content="doomed page",
    ))
...
async def test_empty_page_not_indexed(env):
    ...
    svc, bus, _ = env
    await bus.publish(PageCreated(
        page_id="page-4", space_id=None, title="", content="",
    ))
    assert await svc.search("anything") == []
...
async def test_dm_message_is_indexed_with_scope_message(env):
    ...
    svc, bus, _ = env
    await bus.publish(DmMessageCreated(
        conversation_id="c-1",
        message_id="m-1",
        sender_user_id="u1",
...
async def test_empty_dm_message_not_indexed(env):
    ...
    svc, bus, _ = env
    await bus.publish(DmMessageCreated(
        conversation_id="c-1", message_id="m-2",
        sender_user_id="u1", sender_display_name="Anna",
        recipient_user_ids=("u2",), content="",
    ))

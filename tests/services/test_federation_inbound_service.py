...
async def test_dm_message_persists_and_publishes_event(db, bus, inbound):
    ...
    await inbound._on_dm_message(_event(
    ...
    row = await db.fetchone(
        "SELECT id, content FROM conversation_messages WHERE id=?", ("m-1",),
    )
...
async def test_dm_message_deleted_soft_deletes(db, bus, inbound):
    ...
    await inbound._on_dm_deleted(_event(
        FederationEventType.DM_MESSAGE_DELETED,
        {"message_id": "m-1"},
    ))
    row = await db.fetchone(
        "SELECT deleted FROM conversation_messages WHERE id=?", ("m-1",),
    )
...
async def test_space_post_created_persists(db, bus, inbound):
    ...
    await db.enqueue(
        """INSERT INTO spaces(id, name, owner_instance_id, owner_username,
           ...
           VALUES(?,?,?,?,?,?,?)""",
        ("sp-1", "Space 1", "peer-a", "owner", "aa" * 32,
         SpaceType.HOUSEHOLD.value, JoinMode.INVITE_ONLY.value),
    )
    captured: list[SpacePostCreated] = []
    bus.subscribe(SpacePostCreated, captured.append)

    await inbound._on_space_post_created(_event(
...
async def test_user_status_updated_publishes_bus_event(bus, inbound):
    ...
    await inbound._on_user_status_updated(_event(
...
async def test_dm_reaction_add_and_remove(db, bus, inbound):
    ...
    await inbound._on_dm_reaction(_event(
        FederationEventType.DM_MESSAGE_REACTION,
        {"message_id": "m-1", "user_id": "user-x", "emoji": "👍", "action": "add"},
    ))
    rows = await db.fetchall(
        "SELECT emoji FROM message_reactions WHERE message_id=?", ("m-1",),
    )
    assert [r["emoji"] for r in rows] == ["👍"]

    await inbound._on_dm_reaction(_event(
        FederationEventType.DM_MESSAGE_REACTION,
        {"message_id": "m-1", "user_id": "user-x", "emoji": "👍", "action": "remove"},
    ))
    rows = await db.fetchall(
        "SELECT emoji FROM message_reactions WHERE message_id=?", ("m-1",),
    )
...
async def test_space_post_updated_edits_content(db, bus, inbound):
    ...
    await repo.save("sp-1", Post(
        id="p-1", author="u", type=PostType.TEXT, created_at=now,
        content="old content",
    ))

    await inbound._on_space_post_updated(_event(
...
async def test_space_post_deleted_soft_deletes_and_publishes(db, bus, inbound):
    ...
    repo = SqliteSpacePostRepo(db)
    await repo.save("sp-1", Post(
        id="p-1", author="u", type=PostType.TEXT,
        created_at=datetime.now(timezone.utc), content="x",
    ))
    captured: list[PostDeleted] = []
    bus.subscribe(PostDeleted, captured.append)

    await inbound._on_space_post_deleted(_event(
...
async def test_space_member_join_and_leave(db, inbound):
    ...
    await inbound._on_space_member_joined(_event(
    ...
    await inbound._on_space_member_left(_event(
...
async def test_user_removed_without_existing_remote_is_noop(inbound):
    ...
    await inbound._on_user_removed(_event(
...
async def test_space_comment_created_persists_and_publishes(db, bus, inbound):
    ...
    await repo.save("sp-1", Post(
        id="p-1", author="u", type=PostType.TEXT,
        created_at=datetime.now(timezone.utc), content="post",
    ))
    captured: list[CommentAdded] = []
    bus.subscribe(CommentAdded, captured.append)

    await inbound._on_space_comment_added(_event(
    ...
    row = await db.fetchone(
        "SELECT content FROM space_post_comments WHERE id=?", ("c-1",),
    )
...
async def test_space_report_inbound_persists_remote_report(db, bus):
    ...
    await svc._on_space_report(_event(
    ...
    rows = await db.fetchall(
        "SELECT reporter_user_id, reporter_instance_id, category "
...
async def test_space_report_inbound_noop_without_report_service(db, bus):
    ...
    await svc._on_space_report(_event(
        FederationEventType.SPACE_REPORT,
        {"target_type": "post", "target_id": "p", "category": "spam",
         "reporter_user_id": "u"},
    ))
...
async def test_users_sync_upserts_remote_users(db, inbound):
    ...
    await db.enqueue(
        """INSERT INTO remote_instances(
           ...
           VALUES(?,?,?,?,?,?,?,?,?,?)""",
        ("peer-a", "Peer A", "aa" * 32, "enc", "enc",
         "https://peer/wh", "wh-peer-a",
         "confirmed", "manual", "2026-01-01T00:00:00+00:00"),
    )
    await inbound._on_users_sync(_event(

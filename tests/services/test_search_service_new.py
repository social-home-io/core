...
async def test_index_user_and_find_by_username(env):
    ...
    await svc.index_user(
        user_id="uid-alice", username="alice",
        display_name="Alice Example", bio="loves pumpkin bread",
    )
...
async def test_index_space_and_find_by_name(env):
    ...
    await svc.index_space(
        space_id="sp-garden", name="Garden Club",
...
async def test_delete_user_removes_from_index(env):
    svc, _ = env
    await svc.index_user(user_id="uid-x", username="x", display_name="X",
                         bio="unique-marker")
...
async def test_delete_space_removes_from_index(env):
    svc, _ = env
    await svc.index_space(
        space_id="sp-y", name="Y-club", description="widgets-galore",
    )
...
async def test_index_user_empty_skips(env):
    svc, _ = env
    await svc.index_user(
        user_id="uid-blank", username="", display_name="", bio=None,
    )
...
async def test_search_with_counts_returns_per_scope_counts(env):
    ...
    await svc.index_space(
        space_id="sp1", name="lola-world", description="",
    )
...
async def test_type_posts_expands_to_post_and_space_post(env):
    ...
    await svc._repo.upsert(
        scope="post", ref_id="p1", space_id=None,
        title="", body="pudding recipe",
    )
    await svc._repo.upsert(
        scope="space_post", ref_id="sp1", space_id=None,
        title="", body="pudding tip",
    )
...
async def test_access_filter_drops_non_member_space_post(env):
    ...
    await svc._repo.upsert(
        scope="space_post", ref_id="p1", space_id="sp-private",
        title="", body="secret recipe",
    )
    # Caller is NOT a member of sp-private → hidden.
    result = await svc.search_with_counts(
        "secret",
        caller_user_id="uid-outsider", caller_username="outsider",
    )
    assert result["hits"] == []
...
async def test_access_filter_lets_member_see_space_post(env):
    ...
    result = await svc.search_with_counts(
        "secret",
        caller_user_id="uid-alice", caller_username="alice",
    )
    assert len(result["hits"]) == 1
...
async def test_access_filter_drops_dm_hits_when_wired(env):
    ...
    await svc._repo.upsert(
        scope="message", ref_id="m1", space_id=None,
        title="alice", body="secret dm body",
    )
    result = await svc.search_with_counts(
        "secret",
        caller_user_id="uid-x", caller_username="x",
    )
    assert result["hits"] == []
...
async def test_access_filter_no_op_when_unwired(env):
    ...
    svc, _ = env
    await svc._repo.upsert(
        scope="space_post", ref_id="p1", space_id="sp-x",
        title="", body="content",
    )

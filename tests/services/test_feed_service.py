...
async def test_edit_and_delete(stack):
    ...
    u = await stack.provision_user("pascal")
    p = await stack.feed_svc.create_post(
        author_user_id=u.user_id, type=PostType.TEXT, content="v1",
    )
    updated = await stack.feed_svc.edit_post(
        p.id, editor_user_id=u.user_id, new_content="v2",
    )
...
async def test_non_author_cannot_edit(stack):
    ...
    p = await stack.feed_svc.create_post(
        author_user_id=a.user_id, type=PostType.TEXT, content="x",
    )
...
async def test_admin_can_edit_others(stack):
    ...
    updated = await stack.feed_svc.edit_post(p.id, editor_user_id=b.user_id, new_content="admin")
...
async def test_comments(stack):
    ...
    c = await stack.feed_svc.add_comment(
        p.id, author_user_id=u.user_id, content="nice",
    )
...
async def test_text_post_requires_content(stack):
    ...
    with pytest.raises(ValueError):
        await stack.feed_svc.create_post(
            author_user_id=u.user_id, type=PostType.TEXT, content="   ",
        )
...
async def test_file_post_requires_meta(stack):
    ...
    with pytest.raises(ValueError):
        await stack.feed_svc.create_post(
            author_user_id=u.user_id, type=PostType.FILE,
        )
...
async def test_list_feed_pagination(stack):
    ...
    page2 = await stack.feed_svc.list_feed(
        before=feed[-1].created_at.isoformat(), limit=3,
    )
    assert len(page2) == 2
...
async def test_image_post_accepted(stack):
    ...
    p = await stack.feed_svc.create_post(
        author_user_id=u.user_id, type="image",
        media_url="/media/x.webp", content="caption",
    )
...
async def test_file_post_with_meta(stack):
    ...
    u = await stack.provision_user("a")
    fm = FileMeta(url="/x.pdf", mime_type="application/pdf",
                   original_name="spec.pdf", size_bytes=1024)
    p = await stack.feed_svc.create_post(
        author_user_id=u.user_id, type=PostType.FILE, file_meta=fm,
    )
...
async def test_feed_comment_image_type(stack):
    ...
    p = await stack.feed_svc.create_post(author_user_id=u.user_id, type="text", content="x")
    with pytest.raises(ValueError, match="media_url"):
        await stack.feed_svc.add_comment(p.id, author_user_id=u.user_id, comment_type="image")
...
async def test_feed_comment_parent_wrong_post(stack):
    ...
    p1 = await stack.feed_svc.create_post(author_user_id=u.user_id, type="text", content="a")
    p2 = await stack.feed_svc.create_post(author_user_id=u.user_id, type="text", content="b")
    c = await stack.feed_svc.add_comment(p1.id, author_user_id=u.user_id, content="c")
    with pytest.raises(KeyError, match="parent"):
        await stack.feed_svc.add_comment(p2.id, author_user_id=u.user_id, content="r", parent_id=c.id)
...
async def test_feed_inactive_author_rejected(stack):
    ...
    with pytest.raises(PermissionError, match="not active"):
        await stack.feed_svc.create_post(author_user_id=u.user_id, type="text", content="x")

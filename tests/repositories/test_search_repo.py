...
async def test_upsert_then_search_finds_match(repo):
    await repo.upsert(scope=SCOPE_POST, ref_id="p1", space_id=None,
                      title="", body="hello world from pascal")
...
async def test_upsert_replaces_existing_row(repo):
    await repo.upsert(scope=SCOPE_POST, ref_id="p1", space_id=None,
                      title="", body="version one")
    await repo.upsert(scope=SCOPE_POST, ref_id="p1", space_id=None,
                      title="", body="version two")
...
async def test_upsert_rejects_unknown_scope(repo):
    with pytest.raises(ValueError):
        await repo.upsert(scope="snoop", ref_id="x", space_id=None,
                          title="", body="x")
...
async def test_search_filters_by_scope(repo):
    await repo.upsert(scope=SCOPE_POST, ref_id="p1", space_id=None,
                      title="", body="needle")
    await repo.upsert(scope=SCOPE_SPACE_POST, ref_id="sp1", space_id="sp-A",
                      title="", body="needle")
...
async def test_search_filters_by_space_id(repo):
    await repo.upsert(scope=SCOPE_SPACE_POST, ref_id="s1", space_id="A",
                      title="", body="findme")
    await repo.upsert(scope=SCOPE_SPACE_POST, ref_id="s2", space_id="B",
                      title="", body="findme")
...
async def test_search_limit_clamps_to_100(repo):
    for i in range(120):
        await repo.upsert(scope=SCOPE_POST, ref_id=f"p{i}", space_id=None,
                          title="", body=f"common term row{i}")
...
async def test_search_returns_snippet_with_highlight(repo):
    await repo.upsert(scope=SCOPE_POST, ref_id="p1", space_id=None,
                      title="", body="The quick brown fox jumps over the lazy dog.")
...
async def test_search_diacritic_folding(repo):
    ...
    await repo.upsert(scope=SCOPE_POST, ref_id="p1", space_id=None,
                      title="", body="meet me at the café tonight")

...
def _post_node_payload(type_: str, payload: dict,
                      *, from_node: str, signing_key: bytes):
    body = {
        "type":    type_,
        "from":    from_node,
        "ts":      1700000000,
...
async def test_node_hello_registers_peer(client):
    ...
    canonical, sig = _post_node_payload(
        NODE_HELLO,
        {"node_id": "gfs-node-b", "url": "http://b.test",
         "public_key": pub_hex},
        from_node="gfs-node-b", signing_key=seed,
    )
...
async def test_cluster_sync_unknown_node_is_403(client):
    ...
    canonical = json.dumps(
        {"type": NODE_HEARTBEAT, "from": "ghost", "ts": 1700000000,
         "payload": {}},
...
async def test_apply_sync_space_banned_wins_lww(client):
    ...
    await svc.apply_sync_client(
        action="upsert",
        client_instance={
            "instance_id": "owner.home", "display_name": "O",
            "public_key": "bb" * 32, "endpoint_url": "http://o/wh",
            "status": "active",
        },
    )
    # Ban the space.
    await svc.apply_sync_space(
        action="ban",
        global_space={
            "space_id": "lww-space", "owning_instance": "owner.home",
    ...
    await svc.apply_sync_space(
        ...
        global_space={
            "space_id": "lww-space", "owning_instance": "owner.home",
            "name": "Innocent Space", "status": "active",
        },
    )
...
async def test_admin_cluster_add_and_remove_peer(client):
    ...
    resp = await client.post(
        "/admin/api/cluster/peers", json={"url": "http://peer-c.test"},
    )
...
async def test_cluster_sync_rate_limits_per_node(client):
    ...
    canonical, sig = _post_node_payload(
        ...
        from_node="flood", signing_key=seed,
    )
    await client.post(
        "/cluster/sync", data=canonical,
        headers={"Content-Type": "application/json",
                 "X-Node-Signature": sig, "X-Node-Id": "flood"},
    )
